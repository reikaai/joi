...
Add the `langsmith` marker to `pyproject.toml` markers list if not present. Also ensure `LANGSMITH_TEST_CACHE` environment variable is documented in the test file's module docstring for dual-mode operation.
...
- Dual-mode (real + cached) documented and functional via LANGSMITH_TEST_CACHE
</success_criteria>
